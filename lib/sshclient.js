...
export default class SSHClient {
     ...
     */
    static connectWithKey(host, port, username, privateKey, publicKey, passphrase, callback) {
        return new Promise((resolve, reject) => {
            const result = new SSHClient(host, port, username, { privateKey, publicKey, passphrase }, (error) => {
                if (callback) {
                    callback(error);
                }
                if (error) {
